...
func (rm *Route) HasRequestContentType(contentType string) *Route {
	rm.RequestContentType = append(rm.RequestContentType, contentType)
	return rm
}
...
// ModelOfReflect creates a model of any type.
func ModelOfReflect(t any) Model {
	m := Model{
		Type: reflect.TypeOf(t),
	}
	if sm, ok := t.(CustomSchemaApplier); ok {
		m.s = sm.ApplyCustomSchema
	}
	return m
}
